...
func TestDeviceList(t *testing.T) {
	...
	fd, err := h.context.Storage.GetDeviceByEUI(eui)
	if err != nil {
	...
	if err := h.context.Storage.CreateDevice(tmpDevice, application.eui); err != nil {
		t.Fatal(err)
	}
	first := tmpDevice.DeviceEUI.ToInt64() + 1
	// Create 15 devices with EUIs that will conflict with the next autogenerated
	// eui
	for i := 0; i < 15; i++ {
		d := apiDevice{DeviceEUI: protocol.EUIFromInt64(first).String()}
...
func TestDeviceInfoEndpoint(t *testing.T) {
	...
	err := h.context.Storage.CreateUpstreamData(device1.eui, model.DeviceData{
		...
		RSSI:       -120,
		DataRate:   "SF7BW125",
		GatewayEUI: protocol.EUIFromInt64(0),
	})
